...
- push a hovered citation to [JabRef](https://www.jabref.org/) with `Ctrl + J`: when the citation hover popup shows a bibliography entry, `Ctrl + J` sends it to JabRef's local HTTP server and a status badge on the popup reports whether the entry is new, already in the active library, or in another open library. Pushed in-text citations are marked with a colored dot on the page
